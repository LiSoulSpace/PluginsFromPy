...
async def get_leetcode_daily_question(language: str = "Zh"):
    filePath = os.path.dirname(os.path.abspath(__file__))
        ...
        if i.startswith("img["):
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    image.save(f"{filePath}/temp/tempQuestion{count}.png")
                    msg_list.append(("img", f"{filePath}/temp/tempQuestion{count}.png"))
                    count += 1
        else:
            msg_list.append(i)
    return await util_main.textWithImg_to_img(message_list=msg_list, save_name="DailyQuestionGet.png")
...
if __name__ == '__main__':
    ...
    loop.run_until_complete(get_leetcode_daily_question())
